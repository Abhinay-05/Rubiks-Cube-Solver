...
//    default is 20 moves
    ...
    public void scramble(int length){
        String scrambled = Scrambler.generate(length);
        applyAlgorithm(scrambled);
    }

//    Applies moves to the cube
     public void applyAlgorithm(String algorithm) {

        for (Moves move : AlgorithmParser.parse(algorithm)) {
            applyMove(Moves.valueOf(String.valueOf(move)));
        }
    ...
    public void printCube() {
        ...
        for (int i = 0; i < 3; i++) {
            System.out.print("      ");
            printRow(up.getRow(i));
        }

        // LEFT FRONT RIGHT BACK
        System.out.println("LEFT    FRONT   RIGHT   BACK");
        for (int i = 0; i < 3; i++) {
            printRowInline(left.getRow(i));
            printRowInline(front.getRow(i));
            printRowInline(right.getRow(i));
            printRow(back.getRow(i));
        }
        ...
        for (int i = 0; i < 3; i++) {
            System.out.print("      ");
            printRow(down.getRow(i));
        }
    }
    private void printRow(Colour[] row) {
        for (Colour colour : row) {
            System.out.print(colour + " ");
        }
        System.out.println();
    }
    private void printRowInline(Colour[] row) {
        for (Colour colour : row) {
            System.out.print(colour + " ");
        }
        System.out.print("  ");
